...
1. Fork this repo.
2. In `scripts/fetch_anime.py`, change `USERNAME = "Sabinaya_Sapkota"` to your MAL username.
3. In `index.html`, update the `USERNAME` constant near the top of the `<script>` block to match.
4. Make sure your MAL list is public: **MyAnimeList → Edit Profile → Privacy → Anime List** set to public.
5. In your fork's **Settings → Pages**, set source to "Deploy from a branch" → `main` → `/ (root)`.
6. In the **Actions** tab, run the **Update anime list** workflow manually once to generate your first `data/anime.json`.
7. Visit your new `https://<your-username>.github.io/<repo-name>/`.
...
Built by [Sabinaya Sapkota](https://sabinayasapkota.com.np) · [Portfolio](https://sabinayasapkota.com.np) · [MyAnimeList](https://myanimelist.net/profile/Sabinaya_Sapkota)
